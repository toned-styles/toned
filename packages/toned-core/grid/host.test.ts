...
import { attachGridElement, validateGridElement, type GridHostElement } from './host.ts'
...
const definition = () => defineGrid('card', { columns: [fr(1)], areas: [['body']] })
...
test('child-before-parent ref replacement defers identity checking until commit', () => {
  ...
  const detachFirstChild = attachGridElement(child, { area: first.area('body') })
  const detachSecondChild = attachGridElement(child, { area: second.area('body') })
...
test('committed mismatched identity and wrapper placement still fail', () => {
  ...
  const detachChild = attachGridElement(child, { area: definition().area('body') })
