...
export function systemDefinition(system: TokenStyleDeclaration): string {
  const tokenShapes = Object.entries(system).flatMap(([name, token]) => {
    if (!token || typeof token !== 'object' || !('resolve' in token) || !('values' in token))
      return []
    return [
      [
        name,
...
export function collectManifestConditions(input: unknown, out: Set<string>): void {
  const visited = new WeakSet<object>()
  const key = (name: string) => {
    if (!name.startsWith('@') || name.startsWith('@platform.')) return
    const expression = parseConditionKey(name.slice(1))
    if (expression)
      for (const atom of adHocAtoms(expression)) out.add(`${atom.container}/>=${atom.min}`)
  }
  const walk = (value: unknown): void => {
    ...
    if (record['op'] === 'atom' && typeof record['key'] === 'string') key(record['key'])
    for (const name of Reflect.ownKeys(value)) {
      if (typeof name === 'string') {
        key(name)
        if (name === 'style' || name === '$style' || name === '$grid' || name === '$area') continue
      }
      walk(record[name])
    }
  }
