...
test('explicit virtual import emits CSS and recollects changed declarations', async () => {
  ...
  try {
    ...
    const compile = async () => {
      ...
      const css = result.output.find(asset => asset.type === 'asset' && asset.fileName.endsWith('.css'))
      const html = result.output.find(asset => asset.type === 'asset' && asset.fileName.endsWith('.html'))
      if (css?.type !== 'asset' || html?.type !== 'asset') throw new Error('Missing generated assets')
