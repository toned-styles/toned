  ...
  try {
    ...
    try {
      ;({ stdout } = await exec(
        ...
        { maxBuffer: 2_000_000, timeout: 15_000 },
      ))
    } catch (cause) {
...
test('readt family identity rule resolves aliases and ignores shadowing and non-render factories', async () => {
...
test('explicit reexport contracts and destructured namespace aliases remain import aware', async () => {
  const result = await lint(
    {
  ...
  expect(names(result.diagnostics)).toEqual(['destructure.tsx', 'wrapper.tsx'])
})

test('partial bags are diagnosed only at known hosts and complete spreads retain ownership', async () => {
...
test('global policy detects calls and aliases without banning unrelated APIs or pure token helpers', async () => {
  const result = await lint(
    {
...
test('canonical key fixes respect declaration boundaries and preserve token payloads and collisions', async () => {
  const result = await lint(
    {
...
test('canonical variants are suggestions, with configured wrapper provenance', async () => {
  const result = await lint(
    {
...
test('semantic suggestions are opt-in, static only, and do not rewrite design choices', async () => {
...
test('canonical computed keys and shorthand never change the referenced binding', async () => {
...
test('computed query declarations stay local while dynamic token payloads remain opaque', async () => {
