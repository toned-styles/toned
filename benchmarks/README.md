...
Future optimization should preserve the same differential checks and measure normalization, cache misses and host writes separately. Useful next steps include pre-indexed output operations, dependency-based rule scans, and reducing per-result metadata allocation. Do not replace exact identity with a folded hash to recover the old timing.
